...
from ....tools.smooth import Gaussian

def main(gui, spectrum, header):
    ''' - Smooth / Gaussian

Smoothing by convolving the spectrum with a Gaussian window with a user-specified FWHM.

The upper panel shows the raw spectrum (blue line) and the smoothed spectrum (red line).
The lower panel shows the Gaussian window (blue line) and the FWHM indicator (white marker with blue edge).


Use the cursor to adjust the FWHM of the Gaussian window.
...
Move the cursor to a panel and use the scrolling to zoom in and out and the middle button to center. When zooming in the upper panel, the lower panel will zoom along with it so they have the same scale.'''
    ...
    window, = ax[1].plot(np.arange(spectrum.N)-(spectrum.N-1)/2.,
                         method.window(params['fwhm']), color='blue', drawstyle='steps-mid')
    ax[1].plot(np.arange(spectrum.N)-(spectrum.N-1)/2., np.zeros(spectrum.N), visible=False)
    pfwhm, = ax[1].plot(params['fwhm']/2., np.max(window.get_ydata())/2.,
    ...
    def toggle_point(e):
        ...
        spectrum.set_smooth(method(method.fwhm, flagged))
    ...
    def set_fwhm(e):
        ...
        if e.name == 'pick_event':
            ...
            set_fwhm.active = True
            e = e.mouseevent 

        elif e.name == 'motion_notify_event' and e.button == 1 and set_fwhm.active:
            ...
            pfwhm.set_data(e.xdata,np.max(window.get_ydata())/2.)
            smooth.set_ydata(spectrum.smooth)
            ...
            set_fwhm.active = False

    set_fwhm.active = False

    gui.canvas.mpl_connect('motion_notify_event', set_fwhm)
    gui.canvas.mpl_connect('pick_event', set_fwhm)
